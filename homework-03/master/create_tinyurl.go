...
func CreateTinyURLService(longurl string) (string, error) {
  ...
  for offset = 0; offset + TINYURL_LENGTH < len([]rune(tinyurl)); offset += 1 {
    if _, insert_err = PGConnection.Exec(statement, longurl, tinyurl[offset : offset + TINYURL_LENGTH]); insert_err == nil {
      break
    } else {
